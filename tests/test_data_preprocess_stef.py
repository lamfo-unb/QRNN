...
def test_lagg_3d(dataset, n_lags, price_columns):
    import numpy as np
    from toolz import compose, partial
    
    col_selector = lambda df, price_cols: df[price_cols]
    opener = lambda df, n_steps: df.assign(**{'cnt'+str(time_step): df.shift(-time_step).values for time_step in range(1, n_steps+1)})
    cleaner = lambda df: df.dropna()
    shaper = lambda df,n_steps,n_var: np.reshape(df.iloc[:, :n_steps].values, (df.iloc[:, :n_steps].values.shape[0],n_steps,n_var))
    
    transformations = compose(partial(shaper, n_steps=n_lags, n_var=len(price_columns)),
                              partial(cleaner),partial(opener,n_steps = n_lags),
                              partial(col_selector, price_cols = price_columns))
    
    
    def p(new_df):
        return transformations(new_df)
