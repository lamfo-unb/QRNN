...
def test_adaptive():
    ...
    model, pred_train = adaptive(dataset=train,
                                     price_cols="price1",
                                     target_col="y",
                                     prediction_col="AdaptiveVaR",tau=0.04)

    pred_test = model(test)

    expected_col_train = train.columns.tolist() + ["AdaptiveVaR"]
    expected_col_test = test.columns.tolist() + ["AdaptiveVaR"]
